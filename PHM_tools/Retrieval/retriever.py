...
class RetrieverTool(Tool):
    ...
    Args:
        vector_store: Chroma instance containing embeddings to query.
    """
...
def build_vector_store(persist_directory: str = "./chroma_db") -> Chroma:
    ...
    Args:
        persist_directory: Path where the vector store will be saved.

    Returns:
        Initialized Chroma instance containing document embeddings.
    """
...
def create_retriever_tool(persist_directory: str = "./chroma_db") -> RetrieverTool:
    ...
    Args:
        persist_directory: Location for the persisted vector store.

    Returns:
        A :class:`RetrieverTool` configured with the created store.
    """
