...
Run `demo/gradio_ui_demo.py` to launch a web interface that uses `GradioUI` from `smolagents` so the agent can plan which tools to use interactively.

```bash
python demo/gradio_ui_demo.py
```

## Retrieval-Augmented Generation

`demo/rag_demo.py` demonstrates how to build a small knowledge base with `langchain` and query it using the `RetrieverTool`.
