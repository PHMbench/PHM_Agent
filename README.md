...
The package provides a simple `RetrieverTool` built on top of
`langchain` and `Chroma`. Use `create_retriever_tool()` to load a
demo knowledge base and perform semantic search:

```python
from PHM_tools import create_retriever_tool

retriever = create_retriever_tool()
print(retriever("How can I push a model to the Hub?"))
```

## Web UI

`main.py` accepts a `--ui` flag to launch a `GradioUI` for interactive
experimentation. When used, the agent streams responses via a web
interface similar to the examples in `smolagents-referance`:
